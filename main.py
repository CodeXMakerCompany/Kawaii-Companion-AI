...
from events.handlers import logger_handler, mute, neuma_automations_handler, unmute
...
async def AiAssistant():
    ...
    transcription_task = asyncio.create_task(
        transcriptionService.real_time_transcription(transcript_queue)
    )

    llm_task = asyncio.create_task(
        transcriptionService.transcription_worker(
            transcript_queue, llmService, vtubeService.set_audio_level
        )
    )
