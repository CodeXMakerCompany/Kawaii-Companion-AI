...
function serveStatic(
  ...
  const segs = url.pathname.replace(/^\//, "").split("/").filter((s) => s && s !== "..");
...
server.listen(PORT, () => {
  console.log(`Backend: http://localhost:${PORT} (WebSocket: ws://localhost:${PORT}/client-ws)`);
  console.log(`Place Live2D model in: ${MODELS_DIR}/mao_pro/runtime/ (e.g. mao_pro.model3.json)`);
});
