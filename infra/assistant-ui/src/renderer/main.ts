/**
 * Legacy renderer entry. The app now uses index.tsx (React + DDD use case).
 * Kept for reference; index.html loads renderer/index.js.
 */
...
function setLAppModelConfig(baseUrl: string, modelName: string, kScale: number): void {
...
function setupTestButtons(): void {
  ...
  async function loadCustomExpressions() {
    ...
    for (const action of actionDictionary) {
      try {
        const response = await fetch(`models/SharkGirl/${action.file}.exp3.json`);
  ...
  if (waifuSetup?.hideBackground) {
    ...
    const waitForCanvas = setInterval(() => {
      ...
      if (canvases.length > 0) {
        ...
        console.log("Canvas dimensions:", canvases[0].width, "x", canvases[0].height);
